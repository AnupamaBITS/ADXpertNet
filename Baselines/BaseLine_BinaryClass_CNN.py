...
class SpectralCNN(nn.Module):
    def __init__(self, input_channels, num_filters, kernel_sizes,stride):
        ...
        self.batchnorm1 = nn.BatchNorm1d(num_filters[0])
        self.dropout1 = nn.Dropout(0.2)
        
        
        # Second convolutional layer
        self.conv2 = nn.Conv1d(num_filters[0], num_filters[1], kernel_sizes[1], stride[1])
        self.batchnorm2 = nn.BatchNorm1d(num_filters[1])
        self.dropout2 = nn.Dropout(0.2)
       

        # Third convolutional layer 
        self.conv3 = nn.Conv1d(num_filters[1], num_filters[2], kernel_sizes[2], stride[2])
        self.batchnorm3 = nn.BatchNorm1d(num_filters[2])  # Corrected: New batch norm for conv3
        ...
        self.fc1 = nn.Linear(in_features=15872, out_features=200)
        self.batchnorm_fc1 = nn.BatchNorm1d(200)
...
if __name__ == "__main__":
    ...
    y_train_tensor = torch.tensor(y_balanced, dtype=torch.float32)
    ...
    model = SpectralCNN(input_channels=1, num_filters=[16,64,256], kernel_sizes=[8, 4, 4],stride = [2,2,2]).to(device)
