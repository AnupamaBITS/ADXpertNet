...
class SpectralCNNGRU(nn.Module):
    def __init__(self, input_channels, num_filters, kernel_sizes,stride):
        super(SpectralCNNGRU, self).__init__()
        
        self.conv1 = nn.Conv1d(input_channels, num_filters[0], kernel_sizes[0], stride[0])
        # self.batchnorm1 = nn.BatchNorm1d(128)  # Batch normalization for conv1
        self.dropout1 = nn.Dropout(0.2)
        # self.maxpool1 = nn.MaxPool1d(kernel_size=3, stride=1)
        
        # Second convolutional layer: 32 filters, kernel size 3
        self.conv2 = nn.Conv1d(num_filters[0], num_filters[1], kernel_sizes[1], stride[2])
        # self.batchnorm2 = nn.BatchNorm1d(64)  # Batch normalization for conv2
        self.dropout2 = nn.Dropout(0.2)
        # self.maxpool2 = nn.MaxPool1d(kernel_size=3, stride=1)

        # First convolutional layer: 64 filters, kernel size 5
        self.conv3 = nn.Conv1d(num_filters[1], num_filters[2], kernel_sizes[2], stride[2])
        # self.batchnorm3 = nn.BatchNorm1d(32)  # Batch normalization for conv1
        self.dropout3 = nn.Dropout(0.2)
        # self.maxpool3 = nn.MaxPool1d(kernel_size=3, stride=1)

                 
        # First GRU layer
        self.gru1 = nn.GRU(input_size=256, hidden_size=128, batch_first=True)  # GRU layer with input size 169, hidden size 50
        self.dropout_gru1 = nn.Dropout(0.2)
        # self.maxpool_gru1 = nn.MaxPool1d(kernel_size=3, stride=1)
        
        # Second GRU layer
        self.gru2 = nn.GRU(input_size=128, hidden_size=64, batch_first=True)  # GRU layer with input size 167, hidden size 100
        self.dropout_gru2 = nn.Dropout(0.2)
        # self.maxpool_gru2 = nn.MaxPool1d(kernel_size=3, stride=1)
        ...
        # self.batchnorm_fc1 = nn.BatchNorm1d(500)  # Batch normalization for fc1
        self.dropout_fc1 = nn.Dropout(0.2)
        
        self.fc2 = nn.Linear(500, 300)
        self.batchnorm_fc2 = nn.BatchNorm1d(300)  # Batch normalization for fc2
        self.dropout_fc2 = nn.Dropout(0.2)
        
        # Output layer: 6 output classes
        self.fc3 = nn.Linear(300, 1)  # Final output layer
...
class MultimodalFusionModel(nn.Module):
    ...
    # --- Component branch ---
        ...
        #print("Input spectral shape:", spectral_input.shape)  # should be (B, 518)
        x = spectral_input
        # print("After unsqueeze:", x.shape)  # should be (B, 1, 518)
        x = self.conv_layers(x)
        # # print("After conv:", x.shape)  # should be (B, 256, ~61)
        x = x.permute(0, 2, 1)
        # print("After permute:", x.shape)  # should be (B, ~61, 256)
        x = self.spectral_proj(x)
        # print("After proj:", x.shape)  # should be (B, ~61, 64)

        x = self.spectral_pos_enc(x)    # Add positional encoding
        # print("After projENCODING:", x.shape)
...
if __name__ == "__main__":
    ...
    model = SpectralCNNGRU(input_channels=1, num_filters=[16,64,256], kernel_sizes=[8, 4, 4],stride = [2,2,2]).to(device)
